...
public final class PseudoTeletypewriter {
    ...
    public init?(path:String, arguments:[String], environment:[String] = ["TERM=ansi"]) {
        ...
        if r.result.ok {
            if r.result.isRunningInParentProcess {
                debugPrint("parent: ok, child pid = \(r.result.processID)")
...
//                debugPrint("child: ok")
    ...
    public func waitUntilChildProcessFinishes() {
        ...
        debugPrint("child process quit: pid = \(childpid1)")
    }
}
